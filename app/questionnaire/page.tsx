  ...
  return (
    <main className="relative min-h-screen bg-white flex flex-col items-center justify-center overflow-hidden px-7 py-12">
      ...
      {step === 2 && (
        <button
          onClick={() => setStep(1)}
          className="absolute top-5 right-6 z-10 font-sans text-[11px] tracking-wide text-gray-400"
        >
          {t.q_back}
        </button>
      )}
      ...
      <div className="relative z-10 w-full max-w-sm text-center">
        ...
        <p className="font-sans text-[9px] tracking-[3px] text-sage uppercase mb-2">
          {t.q_step(step, 2)}
        </p>

        {step === 1 ? (
          <>
            <h1 className="font-serif text-[26px] text-gray-800 font-light mb-2">{t.q1_question}</h1>
            <p className="font-sans text-[10px] text-gray-400 tracking-wide mb-5">{t.q1_sub}</p>
          ...
          <>
            <h1 className="font-serif text-[26px] text-gray-800 font-light mb-2">{t.q2_question}</h1>
            <p className="font-sans text-[10px] text-gray-400 tracking-wide mb-5">{t.q2_sub}</p>
            <div className="w-10 h-px bg-gradient-to-r from-transparent via-gold to-transparent mx-auto mb-7" />
...
function ChoiceButton({
  ...
  return (
    ...
    >
      <div>
        <p className="font-serif text-[20px] text-gray-800">{title}</p>
        <p className="font-sans text-[10px] text-gray-400 tracking-wide mt-0.5">{sub}</p>
      </div>
